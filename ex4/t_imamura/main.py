...
def spectrogram(data: np.ndarray, sample_rate: int, ax, fig, indx_ax=None, F_size=1024):
    ...
    if indx_ax:
        ax_spec = ax[indx_ax].pcolormesh(
            ...
            20 * np.log10(spec), 
            vmax=1e-6, 
            cmap="CMRmap")  # dbに変換してスペクトログラムを表示
...
def stft(sample_data:np.ndarray, F_size=1024, OverRap=0.5, window=scipy.signal.hamming) -> tuple[np.ndarray, int]:
    """Calculate stft.
...
def Quefrency(sample_data:np.ndarray, sample_rate:int, F_size:int, OverRap:float, lift_ms=0.018) -> tuple[np.ndarray, np.ndarray, int]:
...
def main():
    ...
    while ac_start + F_size <= N:  # 次のフレーム幅の末端がサンプル数を超えるまでfft
        ...
        for i in range(ac_per.shape[0]-2):  # 最初と最後はピークから除く
            if ac_per[i]<ac_per[i+1] and ac_per[i+1]>ac_per[i+2]: #ピークの候補を持ってくる
    ...
    ax.plot(np.arange(0, counter)*F_size/sample_rate, ac, color="black", label="By autocorrelation method")
    ax1.plot(np.arange(0, counter)*F_size/sample_rate, ac, color="black", label="By autocorrelation method")
    
    #ケフレンシー分析を実行
    F_size = 2048
    OverRap = 0.0 #オーバーラップ率0%
    low_quefrency, high_quefrency, count_fft = Quefrency(sample_data, sample_rate, F_size, OverRap)
    
    #ケフレンシー法で基本周波数を推定
    f0_quef = []
    for high_quef_per in high_quefrency:
        f0_quef.append((sample_rate) / np.argmax(high_quef_per[:F_size//2]))
    ax.plot(np.arange(0, count_fft)*(F_size)/sample_rate, f0_quef, color="blue", label="By cepstrum")
    ax1.plot(np.arange(0, count_fft)*(F_size)/sample_rate, f0_quef, color="blue", label="By cepstrum")
    ...
    ax2.plot(np.linspace(0, sample_rate//2, F_size//2),stft_db[:F_size//2], color="red", label="Spectrogram")
    ax2.plot(np.linspace(0, sample_rate//2, F_size//2), env_quef[:F_size//2], color="blue", label="By cepstrum")
    
    #LPC法でスペクトル包絡を推定
    lpc_parameter = 32 #LPC係数
    ac = auto_correlation(sample_data[extract_frame*F_size:(extract_frame+1)*F_size]*np.hamming(F_size)) #対象フレームに対し自己相関関数を計算
    a, e = LevinsonDurbin(ac, lpc_parameter)
    w, h = scipy.signal.freqz(np.sqrt(e), a, F_size, "whole")
    ax2.plot(np.linspace(0, sample_rate//2, F_size//2),20*np.log10(np.abs(h))[:F_size//2], color="orange", label="By LPC")
