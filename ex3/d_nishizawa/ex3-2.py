...
rows = []
with open(parent.joinpath("data2.csv")) as f:
    reader = csv.reader(f)
...
y_pred ,weights= plot_regression_curve(data[0], data[1], 3)

fig, ax = plt.subplots()

ax.scatter(data[0], data[1], label = "data2", marker='o')
regression_label = build_equation(weights)
ax.plot(data[0], y_pred, color = "red", label = regression_label)
ax.set_xlabel(header[0])
ax.set_ylabel(header[1])
plt.legend()
plt.savefig(parent.joinpath("figs/data2.png"))
plt.show()