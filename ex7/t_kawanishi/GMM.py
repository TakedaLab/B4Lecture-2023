...
def gauss(x:np.ndarray,mean:np.ndarray,var:np.ndarray) ->np.ndarray:
    ...
    b = np.exp(
        np.array(
        [-1/2 * x_m[i] @var_I[i]@x_m[i].T for i in range(n_clusters)]
        )
        )
...
def init(data:np.ndarray, n_clusters:int)->tuple[np.ndarray,np.ndarray,np.ndarray]:
...
def update_gamma(data:np.ndarray,weight:np.ndarray,mean:np.ndarray,var:np.ndarray)->np.ndarray:
    """update gamma.
...
def new_para(data:np.ndarray,gamma:np.ndarray)->tuple[np.ndarray,np.ndarray,np.ndarray]:
...
def log_likelihood(data:np.ndarray,weight:np.ndarray,mean:np.ndarray,var:np.ndarray)->float:
