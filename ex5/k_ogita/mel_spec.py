...
class MelFilterBank:
    ...
    def melfilterbank(self):
        ...
        for m in range(1, self.degree + 1):
            bank_start, bank_center, bank_finish = int(bin[m - 1]), int(bin[m]), int(bin[m + 1])
...
def main():
    ...
    #mfcc_feature = librosa.feature.mfcc(y=data,sr=samplerate,n_mfcc=12)
    #s.draw_spectrogram(
    #    mfcc_feature.T,
    #    ax=ax3,
    #    framesize=framesize,
    ...
    img = ax2_1.imshow(delt[:ncepstrum].T, aspect="auto", extent=[0, len(data)/samplerate, 0, ncepstrum], cmap="rainbow", origin="lower")
    ax2_1.set(title="$\Delta$MFCC sequence", xlabel="time[s]", ylabel="$\Delta$MFCC", yticks=range(0, 13, 4))
    fig2.colorbar(img, aspect=10, pad=0.01, ax=ax2_1, format="%+2.f dB")
    ax2_1.set_ylabel('$\Delta$MFCC')

    ax2_2 = fig2.add_subplot(2, 1, 2)
    img = ax2_2.imshow(deltdelt[:ncepstrum].T, aspect="auto", extent=[0, len(data)/samplerate, 0, ncepstrum], cmap="rainbow", origin="lower")
    ax2_2.set(title="$\Delta\Delta$MFCC sequence", xlabel="time[s]", ylabel="$\Delta\Delta$MFCC", yticks=range(0, 13, 4))
